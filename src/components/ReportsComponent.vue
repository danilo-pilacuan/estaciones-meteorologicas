<template>
  <div class="usersComponent">
    <div class="container">
      <div class="q-pa-md bg-gradient-to-r text-white">
        <q-toolbar>
      ...
      <div class="q-mt-md bg-titles text-white">
        <q-toolbar>
          <q-toolbar-title>Escoja estación</q-toolbar-title>
        </q-toolbar>
      </div>
      ...
      <q-table :rows="usersData" :columns="columns" row-key="id" class="q-mt-md">
        ...
        <template v-slot:body-cell-actions="props">
          <td class="text-right">
            ...
            <q-btn flat icon="touch_app" color="positive" @click="handleSelectStation(props.row)" />
          </td>
        </template>
      </q-table>
      ...
      <div class="q-mt-md bg-titles text-white">
        <q-toolbar>
          <q-toolbar-title>Estación Seleccionada: {{estacionSelected?estacionSelected.numero_serie:"No se ha seleccionado"}}</q-toolbar-title>
        </q-toolbar>
      </div>

      <div class="q-mt-md bg-titles text-white">
        <q-toolbar>
          <q-toolbar-title>Escoja Fechas</q-toolbar-title>
        </q-toolbar>
      </div>

      <div class="q-mt-md q-gutter-sm row justify-start">
        <q-input
          outlined
          label="Fecha Inicio"
          type="date"
        ...
        <q-input
          ...
          class="col-xs-12 col-sm-4 col-md-3"
        />
        <q-btn label="Generar Informe" color="primary" @click="buscarReportes" />
      </div>
    </div>
...
const fechaInicio = ref<string | null>(null); // Variables para las fechas
const fechaFin = ref<string | null>(null);
  const buscarReportes = () => {
  console.log('Buscando reportes desde', fechaInicio.value, 'hasta', fechaFin.value);
  router.push({
    name: 'informe',
    query: {estacion:estacionSelected.value?.numero_serie, fechaInicio: fechaInicio.value, fechaFin: fechaFin.value },
  });
};
const router = useRouter();

const estacionSelected = ref<TEstacionInput|null>(null);
...
const handleSelectStation = (item: TEstacionInput)=>
{
...
//   router.push({
//   name: 'informe',
//   query: { id: '123', fechaInicio: '2023-01-01', fechaFin: '2023-12-31' }
// });

estacionSelected.value = item;
...
.bg-titles {
  background: linear-gradient(to left, #2196F378, #683ab778);
}
</style>
